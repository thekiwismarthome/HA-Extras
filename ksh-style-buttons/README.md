...
│  ├─ byd.yaml                                    ← EV tile: SOC gauge + live %, nav to /byd
...
| **byd** | `nav: byd` | EV tile off the **bydauto** integration: JS‑built car with a SOC gauge ring, colour by charge (charging blue · healthy green · low red · unknown grey), animated bolt + spin ring while plugged in, live `%` + range readout |
